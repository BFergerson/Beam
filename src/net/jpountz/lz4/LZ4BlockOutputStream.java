...
public final class LZ4BlockOutputStream extends FilterOutputStream
{
     ...
     * must be >= 64 and <= 32 M @ param compressor the {@link LZ4Compressor}
     * instance to use to compress data @param checksum the {@link Checksum}
     * instance to use to check data for integrity. @pa ram syncFlush true if
     * pending data should also be flushed on {@link #flush()}
     */
    ...
    public LZ4BlockOutputStream (OutputStream out, int blockSize, LZ4Compressor compressor) {
        this (out, blockSize, compressor, XXHashFactory.fastestInstance ().newStreamingHash32 (DEFAULT_SEED).asChecksum (), true);
    }
    ...
    private void flushBufferedData () throws IOException {
        ...
        actualSentData += HEADER_LENGTH + compressedLength;
        o = 0;
    }
     ...
     * data will be compressed and appended to the underlying
     * {@link OutputStream} before calling {@link OutputStream#flush()} on the
     * underlying stream. Otherwise, this method just flushes the underlying
     * stream, so pending data might not be available for reading until
    ...
    /**
     * Same as {@link #close()} except that it doesn't close the underlying
     * stream. This can be useful if you want to keep on using the underlying
     * stream.
     */
    ...
    public long getActualSentData () {
        return actualSentData;
    }
}
